...
from kotobacore.core.ir import SemanticChunk, Token
...
def _scan_dictionary_matches(
    ...
    cand_by_surface = bundle._cache.get("chunk_cand_by_surface")
    if cand_by_surface is None:  # built once per bundle (was rebuilt per sentence: ~6k setdefault calls each, v0.6.8)
        cand_by_surface = {}
        for cand in candidates:
            cand_by_surface.setdefault(cand.surface, cand)
        bundle._cache["chunk_cand_by_surface"] = cand_by_surface
    for tok in tokens:

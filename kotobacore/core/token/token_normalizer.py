...
def _build_known_hiragana(bundle: DictionaryBundle) -> frozenset[str]:
    ...
    for s in bundle.slang:
        if len(s.surface) >= 2 and all(0x3040 <= ord(c) <= 0x309F for c in s.surface):
            words.add(s.surface)
    for se in getattr(bundle, "sentiment", []):  # v0.5: evaluative words (いまいち / だめ)
        if len(se.surface) >= 2 and all(0x3040 <= ord(c) <= 0x309F for c in se.surface):
            words.add(se.surface)
...
_VERB_TAILS: tuple[str, ...] = (
    ...
    # v0.6.3 (N4): conditional 仮定形 so 走れば / 書けば / 食べれば assemble as one verb token
    "れば", "けば", "げば", "せば", "てば", "ねば", "べば", "めば", "えば",
)

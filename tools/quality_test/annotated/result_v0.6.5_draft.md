# Annotated eval — KotobaCore 0.6.4 (300 items)

| Tokenization (73 items) | P 0.8934 / R 0.9224 / F1 0.9077 |
| entity_surface | P 0.731 / R 0.6092 / F1 0.6646 |
| entity_typed | P 0.7172 / R 0.5977 / F1 0.652 |
| Sentiment accuracy | 61.7% (n=300) |
| Emotion accuracy | 36.8% (n=182), canary FP 22.7% (n=22) |
| Intent accuracy | 45.0% (n=300) |
| Sentiment target accuracy | 45.5% (n=33) |
| Timing | mean 5.88 ms / p95 3.26 ms |

## Per genre

| genre | sentiment | emotion | intent |
|---|---|---|---|
| business | 65.0% | 34.8% | 48.3% |
| conversation | 56.7% | 41.5% | 35.0% |
| literary | 83.3% | 35.1% | 71.7% |
| sns | 53.3% | 43.5% | 38.3% |
| tech | 50.0% | 25.7% | 31.7% |
...
- `tech-034` emotion: gold=['admiration'] sys=trust 障害の復旧、佐藤さんの判断が的確だった。
...
- `tech-059` sentiment: gold=negative sys=None レガシーコードを触るのは嫌だが、避けては通れない。
- `tech-059` emotion: gold=['disgust'] sys=irritation レガシーコードを触るのは嫌だが、避けては通れない。
- `tech-060` sentiment: gold=negative sys=None パスワードを平文で保存してるって聞いてぞっとした。

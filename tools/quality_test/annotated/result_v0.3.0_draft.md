# Annotated eval — KotobaCore 0.3.0 (300 items)

| Tokenization (73 items) | P 0.8907 / R 0.9196 / F1 0.9049 |
| entity_surface | P 0.5385 / R 0.0402 / F1 0.0749 |
| entity_typed | P 0.4615 / R 0.0345 / F1 0.0642 |
| Sentiment accuracy | 53.7% (n=300) |
| Emotion accuracy | 24.7% (n=182), canary FP 22.7% (n=22) |
| Intent accuracy | 13.0% (n=300) |
| Timing | mean 2.72 ms / p95 2.08 ms |
...
| conversation | 53.3% | 19.5% | 20.0% |
| literary | 60.0% | 24.3% | 1.7% |
| sns | 41.7% | 32.6% | 15.0% |
| tech | 48.3% | 17.1% | 10.0% |
...
- `sns-002` sentiment: gold=negative sys=positive 新しいiPhone買ったけど正直そこまで感動しない。
- `sns-003` sentiment: gold=None sys=positive 明日のライブ、わくわくが止まらない！
...
- `sns-009` sentiment: gold=None sys=mixed え、ブルースカイズ再結成ってマジ？？
...
- `sns-010` sentiment: gold=None sys=positive 3年ぶりに実家帰ったら犬が覚えててくれて泣いた🥲
- `sns-011` tokens: gold=['正直', '、', '今回', 'の', 'アップデート', 'は', '改悪', 'だ', 'と', '思う', '。'] sys=['正直', '、', '今回', 'の', 'アップデート', 'は', '改悪だ', 'と', '思う', '。'] 
- `sns-011` sentiment: gold=negative sys=positive 正直、今回のアップデートは改悪だと思う。
...
- `sns-014` sentiment: gold=None sys=positive フォロワー1万人突破！！みんなありがとう🙏
...
- `sns-016` sentiment: gold=None sys=positive 【拡散希望】10月5日に代々木公園で保護猫譲渡会やります！
- `sns-017` sentiment: gold=None sys=positive ぴえん🥺推しのグッズ売り切れてた…
- `sns-017` emotion: gold=['sadness'] sys=admiration ぴえん🥺推しのグッズ売り切れてた…
- `sns-018` sentiment: gold=None sys=positive ＡＢＣストア１２３号店限定のスニーカー、ゲットしたぜ😎
...
- `sns-025` sentiment: gold=None sys=negative 嬉しくないと言えば嘘になるけど、素直に喜べない。
...
- `sns-030` sentiment: gold=positive sys=mixed この写真、加工なしでこれ？信じられない😳
...
- `sns-033` sentiment: gold=positive sys=mixed 全然期待してなかったけど、この漫画めっちゃ面白い！
...
- `sns-036` sentiment: gold=positive sys=mixed ワンワン吠えてる犬、めっちゃ可愛い🐶
- `sns-036` emotion: gold=['joy'] sys=exaggeration ワンワン吠えてる犬、めっちゃ可愛い🐶
- `sns-037` sentiment: gold=None sys=positive いいね押しすぎて指が疲れた笑
- `sns-038` sentiment: gold=None sys=positive これは同意しかない。ほんとそれ。
...
- `sns-040` sentiment: gold=None sys=positive 「感動」って名前のパン屋、駅前にできてた。
...
- `sns-045` sentiment: gold=positive sys=negative え、待って、無理、可愛すぎる🫠
...
- `sns-048` sentiment: gold=None sys=positive フォロバありがとうございます！これからよろしくお願いします🙇
...
- `sns-054` sentiment: gold=None sys=mixed やばい、財布落とした。誰か見てない？
...
- `sns-059` sentiment: gold=None sys=mixed 弟が第一志望に受かった！家族全員で泣いた😭
...
- `business-019` sentiment: gold=None sys=positive 「感動」は当社の新型加湿器のブランド名です。
- `business-019` canary: gold=None sys=moved 「感動」は当社の新型加湿器のブランド名です。
- `business-022` sentiment: gold=negative sys=positive 正直なところ、今回の提案には期待しておりません。
...
- `business-034` sentiment: gold=None sys=negative 佐々木課長は退職の報告を聞いて動揺されていました。
...
- `business-042` sentiment: gold=None sys=positive 今回のプロジェクトは、社員一同わくわくしながら取り組んでおります。
...
- `business-046` sentiment: gold=negative sys=positive 正直、この見積もりには納得しかねます。
...
- `business-054` sentiment: gold=None sys=negative 「怒り」は弊社ゲーム部門の新作タイトルです。
...
- `tech-014` sentiment: gold=negative sys=mixed LLMの出力をそのまま本番に流すのは怖すぎる。
...
- `tech-020` sentiment: gold=None sys=mixed 「驚き」はこのゲームエンジンのパーティクル機能の名称です。
...
- `tech-023` sentiment: gold=negative sys=positive CIが40分もかかるのはさすがに耐えられない。
- `tech-023` emotion: gold=['irritation'] sys=admiration CIが40分もかかるのはさすがに耐えられない。
- `tech-025` sentiment: gold=negative sys=positive 認証周りの実装、正直不安しかない。
...
- `tech-048` sentiment: gold=negative sys=positive AIチャットに聞いたら堂々と間違ったコードを出してきて笑った。
...
- `tech-054` sentiment: gold=negative sys=positive 依存パッケージが300個超えてるの、さすがに多すぎでは？
